  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        fontSize: 16, fontWeight: 900, letterSpacing: '0.22em', textTransform: 'uppercase',
      ...
      {activeToast && (
        <div style={{
          ...
          {activeToast.type === 'rent'    && `💸 Rent ₹${Number(activeToast.amount).toLocaleString('en-IN')}${activeToast.partial ? ' (partial)' : ''}`}
          {activeToast.type === 'buy'     && `🏠 Purchased! ₹${Number(activeToast.amount).toLocaleString('en-IN')}`}
...
export const MonopolyBoard = React.memo(function MonopolyBoard({
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%', minWidth: 0, minHeight: 0 }}>
      ...
      {activeTrade && activeTrade.toPlayerId === myId && !showTradeModal && (
        <div
